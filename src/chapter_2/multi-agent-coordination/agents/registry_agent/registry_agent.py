"""
Registry Agent — manages agent discovery, registration, and health monitoring.

Maintains the agent registry, handles heartbeat monitoring, and provides
discovery services so agents can find and communicate with each other.

Deployed as A2A service on Amazon Bedrock AgentCore. Port 9100.
"""
...
def load_config() -> dict:
    secret_name = os.getenv("SECRET_NAME", "multi-agent-coord/agent-config")
...
async def _initialize_agent():
    ...
    log_info("Initializing Registry Agent...")
    tools = []
    ...
    system_prompt = """You are the Registry Agent for a multi-agent coordination system.

Your responsibilities:
1. Register new agents — store their capabilities, endpoints, and metadata
2. Discover agents — find agents by capability, role, or name
3. Monitor health — track heartbeats, detect unavailable agents
4. Manage lifecycle — handle deregistration and capability updates
5. Route messages — help agents find the right peers for collaboration

When registering agents:
- Validate capability declarations
- Assign unique agent IDs if not provided
- Store registration timestamp

When discovering agents:
- Match by capability keywords, role, or specialization
- Filter out agents that have missed heartbeats (>60s timeout)
- Return endpoint, capabilities, and health status

Be precise and return structured data. Always include agent_id and status in responses."""

    _graph = create_agent(llm, tools, system_prompt=system_prompt)
    log_info("Registry Agent ready")
    return _graph
...
@app.get("/health")
async def health():
    return {"status": "healthy", "agent": "registry-agent"}


if __name__ == "__main__":
    uvicorn.run(app, host="0.0.0.0", port=int(os.getenv("AGENT_PORT", "9100")), log_level="warning")
